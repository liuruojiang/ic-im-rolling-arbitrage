...
- v1.2 GitHub/Gmail 日报只允许由`run_ic_im_v1_2_github_digest.py`恢复哈希链账本并补到最近已完成交易日；盘中邮件必须同时取得IC/IM当天完整行情，且不得把盘中快照写入账本。自动化仓库必须恢复最近成功的`ic-im-v1-2-ledger`工件；失败邮件不得展示陈旧目标为当天信号。
