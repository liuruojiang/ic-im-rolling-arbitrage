...
def rule_manifest() -> dict[str, Any]:
    return {
        ...
        "provenance": {
            "ic_module": "ic_mainline_v1_3.py",
            "im_module": "im_mainline_v1_3.py",
            "spec": str(SPEC_PATH),
            "source_sha256": {
                "ic_module": _sha256(ROOT / "ic_mainline_v1_3.py"),
                "im_module": _sha256(ROOT / "im_mainline_v1_3.py"),
...
def load_authoritative_local_state() -> tuple[dict[str, pd.DataFrame], dict[str, Any]]:
    ...
    if not date_parity:
        raise RuntimeError("IC and IM v1.3 target dates are not identical")
