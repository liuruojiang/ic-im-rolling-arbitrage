...
- 真实IM/MO匹配消融仅覆盖2022-07-22以来样本；35%–40%高IV区间出现的收益改善主要来自少持Put，不是降仓改善尾部风险，分类为`return_only_not_defensive`观察项。
- 2015年以来的扩展只使用中证1000方向代理、理论Put和QVIX缩放代理IV，必须与真实IM/MO证据分开；它不能证明可成交结果。
- 三阶段预注册、脚本和正式测试保留用于后续研究；冻结V2、v1.2、Poe和GitHub/Gmail信号规则均未因此改变，也未获实盘授权。
...
- HTML正文按IC、IM分别展示当前值、下一交易日目标，以及动量、估值网格、Put、Call和展期五条原因链；原因直接取自当次已审计信号字段，不能用硬编码结果或缺失值补写。
