"""Build deterministic IC/IM v1.2 fixed reference-return curves.
...
IC_PUT_DAILY = ROOT / "outputs" / "ic_roll_momentum_stage2_put_v2" / "daily_nav.csv.gz"
IC_GRID_DAILY = ROOT / "outputs" / "ic_roll_momentum_stage3_grid_v1" / "daily_nav.csv.gz"
IC_TARGET = ROOT / "outputs" / "ic_mainline_v1_2" / "target_schedule.csv.gz"
IM_COMPONENTS = (
    ROOT
...
IM_TARGET = ROOT / "outputs" / "im_mainline_v1_2" / "target_schedule.csv.gz"
IM_REAL_PUT_AUDIT = (
    ROOT
    / "quant_param_scan_runs"
    / "20260823_im_grid160_put_carry_scan_v23"
...
POE_BOT = ROOT / "poe_ic_im_mainline_v1_2_bot.py"


def prelisting_basis_disclosure_lines() -> list[str]:
    """Disclosure that must accompany every future v2 record artifact."""
...
def reproducibility_sources() -> tuple[dict[str, str], dict[str, Any]]:
    ...
    code_paths = {
        ...
        "ic_grid_module": Path(ic_grid.__file__).resolve(),
        "im_proxy_module": Path(im_proxy.__file__).resolve(),
        "ic_target_module": ROOT / "ic_mainline_v1_2.py",
        "im_target_module": ROOT / "im_mainline_v1_2.py",
    }
    code_hashes = {
    ...
    constants: dict[str, Any] = {
        ...
        "supersedes_builder_output": V1_OUTPUT.name,
        "v1_output_policy": "immutable_read_only",
    }
...
def build_ic() -> tuple[pd.DataFrame, dict[str, Any]]:
    put = pd.read_csv(
        IC_PUT_DAILY,
        parse_dates=["date"],
        usecols=[
            "date",
            "put_data_layer",
            "put_momentum_valuation_only_ret",
            "put_momentum_valuation_only_cash_weight",
        ],
    )
    grid = pd.read_csv(
        IC_GRID_DAILY,
    ...
    target = pd.read_csv(
        IC_TARGET,
        parse_dates=["date"],
        usecols=["date", "grid_held_eod", "total_ic_units", "total_put_target_delta"],
    )
    frame = put.merge(grid, on="date", validate="one_to_one").merge(
        target, on="date", validate="one_to_one"
    )
    frame = _validate_dates(frame, "IC")
    ...
    base_cash = frame["put_momentum_valuation_only_cash_weight"].astype(float)
    base_pre_cash = (
        frame["put_momentum_valuation_only_ret"].astype(float)
        - base_cash * ic_grid.CASH_DAILY
    )
    cash = base_cash - ic_grid.MARGIN_RATE * frame["grid_held_eod"].astype(float)
    ...
    result = pd.DataFrame(
        {
            ...
            "data_layer": frame["put_data_layer"].astype(str),
        }
    )
    ...
    audit = {
        ...
        "formula": "IC v1.2 executed Put path plus independent grid increment and cash adjustment",
    }
    return result, audit
...
def metrics(frame: pd.DataFrame, start: pd.Timestamp) -> dict[str, Any]:
...
def build_metrics(curves: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for product, frame in curves.items():
        for window, start in windows.items():
            rows.append({"product": product, "window": window, **metrics(frame, start)})
...
def main() -> None:
    ...
    try:
        ...
        lines = [
            "# IC/IM v1.2 定值参考曲线 v2",
            "",
            f"状态：`{STATUS}`；不生成订单，不改变冻结V2、1.2目标日程或既有v1定值输出。",
            "",
        ...
        lines.extend(
            [
                "",
                "IC使用已执行的1.2 Put逐日路径并叠加独立网格；IM使用0.5核心+0.5动量、"
