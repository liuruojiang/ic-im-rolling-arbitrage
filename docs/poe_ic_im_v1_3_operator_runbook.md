...
## 2026-09-07 Put月度修复：本地正式入口

当前本地构建`v1.3-20260907-r7-put-monthly-fix1`，账本schema 3/r7不变。核心与动量Put均独立月度重选，目标月份相同不豁免行权价重选；非月度沿用原合约调整数量，从零进入重新选约。95%以`core_current`的IM报价为参考，不能使用指数或仅预告的下一合约。信号输出`im_put_execution_revision`及月度计划执行日；计划和目标不等于已成交。

9月7日起日报校验要求当前修复标识，禁止拿旧版本信号套上新构建号交付。历史哈希链记录仍可读，不追改其目标或持仓锚点。策略代码发布提交为`a3a6db43bb56c19200d4ea3625842ce29ecfda1d`；GitHub日报固定检出该提交，纠正版验收运行`34111608341`已完成信号、独立账本、Gmail和送达标记。Codex 14:30任务保持启用并校验同一构建和执行标识。Poe云服务不属于本次日报切换范围，其部署状态仍按后文历史记录判断。

旧IM r6嵌入净值已撤回有效绩效资格，默认绩效查询停止展示其数字/图。只有显式历史审计参数`allow_invalidated_im_history=True`可读取旧路径；不改变冻结IC参考。修复后研究绩效见`quant_param_scan_runs/20260907_im_round_completion_v1/record.md`，不能冒充已续接到正式账本的收益。

验收与回退：见`outputs/im_formal_monthly_correction_20260907/record.md`；代码修复前备份位于`.codex_backups/20260907_161311/`，包含此前未提交改动。未改实际r7账本；测试用独立副本。回退代码不可覆盖或倒退运行后新增的历史日志。
...
## 2026-09-04 GitHub 收盘源切换验收

交付修订 `20260904-crossday-transport-2` 保留断连/超时单品种一次重试，并将既有 OHLCV 完整校验前移到每个来源被选中之前。主源过期或缺日不再阻止备用源尝试；备用源仍须通过同一历史日期指纹、成交量、价格及最新交易日检查，不拼接、不补零、不用盘中价替代收盘。

GitHub 固定策略提交，并将真实 SHA 写入工件；`--expected-market-date` 在补账前阻止把昨日收盘作为今日交付。2026-09-04 15:26 首次云端验收已完成 r6 到 r7 的独立迁移，但因 IC/IM 主源均止于 09-03 被正确拦截；这是失败验收，不是成功日报。后续必须以最终 workflow conclusion、r7 工件日期/哈希链及 Gmail 步骤和送达标记验收，不以工作流显示名称推定历史运行版本。
