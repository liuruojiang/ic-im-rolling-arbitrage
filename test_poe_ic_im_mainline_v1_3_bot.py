...
import im_mainline_v1_3 as im_local
...
def _full_ic_history(end: str) -> pd.Series:
    import ic_mainline_v1_3 as ic_local

    fixed = pd.read_csv(ic_local.CSI500_OHLCV_PATH, parse_dates=["date"])["date"]
    extension = pd.bdate_range("2026-08-17", end)
    index = pd.DatetimeIndex([*fixed, *extension]).drop_duplicates().sort_values()
    return pd.Series(np.linspace(6000.0, 8000.0, len(index)), index=index)


def test_poe_source_compiles_and_executes_without_file(monkeypatch):
    source = Path(bot.__file__).read_text(encoding="utf-8")
    compile("# Poe bootstrap\n" + source, "poe_v13.py", "exec")
    module = types.ModuleType("poe_v13_without_file")
...
def test_v13_im_parent_targets_keep_three_put_momentum_floor_and_new_grid():
...
def test_v13_momentum_score_matches_a_share_v13_local_formula(product):
    ...
    actual = bot.calc_v13_momentum_score(product, close)
    np.testing.assert_allclose(actual, expected, rtol=0.0, atol=1e-12, equal_nan=True)


def test_v13_im_complete_momentum_schedule_matches_local_authority():
    ohlcv = pd.read_csv(im_local.CSI1000_OHLCV_PATH, parse_dates=["date"]).set_index("date")
    local = im_local.build_momentum_schedule(ohlcv).set_index("date")
    poe = bot.v13_momentum_schedule("IM", ohlcv["close"], ohlcv)
    np.testing.assert_allclose(
        poe["execution_weight"], local["momentum_execution_weight"], atol=1e-12, rtol=0.0
    )
    np.testing.assert_allclose(
        poe["signal_target"], local["momentum_signal_target"], atol=1e-12, rtol=0.0
    )
    np.testing.assert_allclose(poe["volume_ratio"], local["volume_ratio"], atol=1e-12, rtol=0.0, equal_nan=True)


def test_v13_ic_complete_momentum_schedule_matches_local_authority():
    import ic_mainline_v1_3 as ic_local

    ohlcv = pd.read_csv(ic_local.CSI500_OHLCV_PATH, parse_dates=["date"]).set_index("date")
    local = ic_local.build_momentum_schedule(ohlcv).set_index("date")
    poe = bot.v13_momentum_schedule("IC", ohlcv["close"], ohlcv)
    np.testing.assert_allclose(
        poe["execution_weight"], local["momentum_execution_weight"], atol=1e-12, rtol=0.0
    )
    np.testing.assert_allclose(
    ...
    np.testing.assert_allclose(
        poe["base_dd_for_gate"], local["base_dd_for_gate"], atol=1e-12, rtol=0.0
    )


def test_v13_ic_rejects_truncated_history_that_resets_nav_high_watermark():
    import ic_mainline_v1_3 as ic_local

    ohlcv = pd.read_csv(ic_local.CSI500_OHLCV_PATH, parse_dates=["date"]).set_index("date")
    truncated = ohlcv.tail(2000)
    with pytest.raises(RuntimeError, match="截断历史会重置NAV防御高水位"):
        bot.v13_momentum_schedule("IC", truncated["close"], truncated)


def test_v13_ohlcv_rejects_missing_fixed_history_day_and_amount_substitution():
    frame = pd.read_csv(im_local.CSI1000_OHLCV_PATH, parse_dates=["date"]).set_index("date")
    clock = datetime(2026, 8, 14, 16, 0, tzinfo=bot.BEIJING)
    missing = frame.drop(pd.Timestamp("2026-07-16"))
    with pytest.raises(RuntimeError, match="交易日索引不完整或被改写"):
        bot._validate_v13_ohlcv("IM", missing, clock)
    amount = frame.copy()
    amount["volume"] = amount["volume"] * amount["close"]
    with pytest.raises(RuntimeError, match="成交量单位/数量级异常"):
        bot._validate_v13_ohlcv("IM", amount, clock)


def test_intraday_partial_kline_volume_is_frozen_to_previous_confirmed_state(monkeypatch):
    frame = pd.read_csv(im_local.CSI1000_OHLCV_PATH, parse_dates=["date"]).set_index("date")
    extension = pd.bdate_range("2026-08-17", "2026-09-03")
    previous_close = float(frame["close"].iloc[-1])
    for offset, day in enumerate(extension, start=1):
        close = previous_close + offset
        frame.loc[day, ["open", "high", "low", "close", "volume"]] = [
            close, close * 1.01, close * 0.99, close, 30_000_000_000.0
        ]
    frame.loc[pd.Timestamp("2026-09-03"), "volume"] = 78_316_106.0
    frame.attrs["source"] = "Eastmoney"
    monkeypatch.setattr(bot, "fetch_ohlcv_history", lambda _product: frame.copy())
    monkeypatch.setattr(
        bot,
        "fetch_live_price_quote",
        lambda _product: {
            "price": float(frame.loc[pd.Timestamp("2026-09-03"), "close"]),
            "source": "测试实时源",
            "source_date": date(2026, 9, 3),
            "source_time": "10:00:00",
        },
    )
    live = bot.live_proxy("IM", datetime(2026, 9, 3, 10, 0, tzinfo=bot.BEIJING))
    assert live["momentum_volume_placeholder"] is True
    assert live["momentum_volume_ratio"] > 0.9


def test_live_proxy_reconciles_current_and_next_momentum_and_put(monkeypatch):
    history = _full_ic_history("2026-08-21")
    monkeypatch.setattr(
        bot,
        "fetch_live_price_quote",
        lambda _product: {
    ...
    monkeypatch.setattr(bot, "fetch_ohlcv_history", lambda _product: _as_ohlcv(history))
    live = bot.live_proxy("IC", datetime(2026, 8, 23, 12, tzinfo=bot.BEIJING))
    assert live["momentum_current_weight"] in {0.0, 0.25, 0.5, 1.0}
    assert live["momentum_next_weight"] in {0.0, 0.25, 0.5, 1.0}
    assert live["v13_put_target_delta"] == pytest.approx(
        live["v13_core_put_delta"] + live["v13_momentum_put_delta"]
    )
...
def test_historical_replay_truncates_future_history_and_never_uses_live_quote(
    ...
    history = _full_ic_history("2026-08-26")
    monkeypatch.setattr(bot, "fetch_ohlcv_history", lambda _product: _as_ohlcv(history))
...
def test_v13_im_live_continuation_scales_options_to_half_core_sleeve():
...
def test_session_bridge_current_legs_are_taken_from_verified_anchor():
    live = {
        ...
        "v13_current_put_delta": 1.0,
        "v13_current_core_put_delta": 0.5,
        "v13_current_momentum_put_delta": 0.5,
        "current_core_put_driver": "错误重算",
        "current_momentum_put_driver": "错误重算",
    }
    anchored = bot._apply_next_unverified_session_anchor("IC", live)
    assert anchored["momentum_current_weight"] == 0.25
    assert anchored["grid_current_units"] == 0.0
    assert anchored["v13_current_put_delta"] == 0.25
    assert anchored["v13_current_core_put_delta"] == 0.25
    assert anchored["v13_current_momentum_put_delta"] == 0.0
